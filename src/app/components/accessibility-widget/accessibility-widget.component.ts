...
@Component({
  ...
  template: `
    ...
    <button (click)="toggleMenu()"
      class="fixed left-6 bottom-6 z-[60] inline-flex items-center gap-3 bg-white text-brand-dark px-5 py-3.5 rounded-full shadow-[0_15px_40px_-10px_rgba(75,123,245,0.4)] hover:-translate-y-1 hover:shadow-brand-blue/40 border border-gray-100 transition-all duration-300 focus:outline-none group"
      aria-label="Menú de Accesibilidad">
      
      <div class="relative bg-brand-blue/10 p-2.5 rounded-full text-brand-blue group-hover:bg-brand-blue group-hover:text-white transition-colors duration-300">
         <!-- Efecto de pulso para llamar la atención sutilmente -->
         <span class="absolute inset-0 rounded-full animate-ping bg-brand-blue/30 opacity-75"></span>
      ...
      <span class="font-extrabold text-sm tracking-wide pr-1">Accesibilidad</span>
    </button>

    <!-- Menú de opciones -->
    <div *ngIf="isOpen" class="fixed left-6 bottom-28 z-[60] w-72 bg-white rounded-2xl shadow-2xl p-5 border border-gray-100 animate-fade-in-up">
